...
public class Projectile extends GameObject {
    ...
    /**
     * This update function controls the movement of this projectile, scaled with deltaTime so that
     * framerate does not affect gameplay
     *
     * @param deltaTime time for last frame to execute
     */
    public void update(float deltaTime) {
        float change = movementSpeed * deltaTime;
        RealWorldCoordinates dir = new RealWorldCoordinates(
            targetPosition.getX() - getRealWorldCoordinates().getX(),
            targetPosition.getY() - getRealWorldCoordinates().getY());
        // normalise vector
        double len = Math.sqrt(Math.pow(dir.getY(), 2) + Math.pow(dir.getX(), 2))
            / 10; // need to divide by 10 otherwise you can get 0,0 when normalising, because not using floats
        RealWorldCoordinates norm = new RealWorldCoordinates((int) (dir.getX() / len),
            (int) (dir.getY() / len));
        RealWorldCoordinates movedBy = new RealWorldCoordinates((int) (norm.getX() * change),
            (int) (norm.getY() * change));
        RealWorldCoordinates newPos = new RealWorldCoordinates(
            getRealWorldCoordinates().getX() + movedBy.getX(),
            (getRealWorldCoordinates().getY() + movedBy.getY()));
    ...
    private float distance(RealWorldCoordinates current, RealWorldCoordinates target) {
        RealWorldCoordinates delta = new RealWorldCoordinates(current.getX() - target.getX(),
